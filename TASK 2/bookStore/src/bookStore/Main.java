...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Connection connection = DriverManager
					.getConnection("jdbc:mysql://localhost:3306/ebookstore_db?useSSL=false", "otheruser", "swordfish");
	...
	public static void enterBook(Statement statement) throws SQLException {
		...
		System.out.print("Please enter book Qty:");
		int bookQty = scan.nextInt();
		scan.nextLine();

		// INSERT INTO Books
		statement.executeUpdate("INSERT INTO books VALUES(" + bookId + " , '" + bookTitle + "', '" + bookAuthor + "', "
				+ bookQty + ")");
	...
	public static void updateBook(Statement statement) throws SQLException {
		...
		// Ask if user knows which book they want to amend
		System.out.println("Do you know which book you want to update by ID? \n1. YES \n2. NO");
		int userKnowsId = scan.nextInt();
		scan.nextLine();
		...
		ResultSet results = statement.executeQuery("SELECT id, Title, Author, Qty FROM books WHERE id =" + bookId);
		...
		case "1":
			...
			String bookTitle = scan.nextLine();
			statement.executeUpdate("UPDATE books SET Title = '" + bookTitle + "' WHERE id = " + bookId);
			break;
		...
		case "2":
			...
			System.out.print("Please enter book Author:");
			String bookAuthor = scan.nextLine();
			statement.executeUpdate("UPDATE books SET Author = '" + bookAuthor + "' WHERE id = " + bookId);
			break;
		...
		case "3":
			...
			int bookQty = scan.nextInt();
			scan.nextLine();
			statement.executeUpdate("UPDATE books SET Qty = " + bookQty + " WHERE id = " + bookId);
			break;
	...
	public static void deleteBook(Statement statement) throws SQLException {
		...
		// Ask if user knows which book they want to delete
		System.out.println("Do you know which book you want to delete by ID? \n1. YES \n2. NO");
		int userKnowsId = scan.nextInt();
		scan.nextLine();
	...
	public static void searchBook(Statement statement) throws SQLException {
		...
		case "2":
			...
			String bookTitle = scan.nextLine();
			results = statement
					.executeQuery("SELECT id, Title, Author, Qty FROM books WHERE Title ='" + bookTitle + "'");
			displayResults(results);
			break;
		...
		case "3":
			...
			String bookAuthor = scan.nextLine();
			results = statement
					.executeQuery("SELECT id, Title, Author, Qty FROM books WHERE Author ='" + bookAuthor + "'");
			displayResults(results);
			break;
		...
		case "4":
			...
			int bookQty = scan.nextInt();
			scan.nextLine();
			results = statement.executeQuery("SELECT id, Title, Author, Qty FROM books WHERE Qty >" + bookQty);
			displayResults(results);
			break;
		...
		case "5":
			...
			scan.nextLine();
			results = statement.executeQuery("SELECT id, Title, Author, Qty FROM books WHERE Qty <" + bookQtyBelow);
			displayResults(results);
			break;

		case "6":
			// Display all the books in library
			System.out.print("Are you sure you want to display all the books in the library?: \n1. Yes \n2. No");
